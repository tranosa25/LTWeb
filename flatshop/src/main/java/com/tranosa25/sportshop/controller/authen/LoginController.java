...
public class LoginController {
	...
	/*
	 * @PostMapping(value = "/login") public String login (HttpSession
	 * session, @RequestParam(name = "account") String account,
	 * 
	 * @RequestParam(name = "password") String password, HttpServletRequest request)
	 * { try { UserDTO userDTO =
	 * userService.findByEmailOrPhoneAndPassword(account,password, true); String
	 * username = account.split("@")[0]; session.setAttribute("username", username);
	 * session.setAttribute("user",userDTO); return "redirect:/client/home"; } catch
	 * (Exception e) {
